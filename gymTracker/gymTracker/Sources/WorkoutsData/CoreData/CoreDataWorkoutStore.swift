...
public final class CoreDataWorkoutStore: Sendable {
	private static let modelName = "WorkoutStore"
	...
	public convenience init(storeURL: URL, contextQueue: ContextQueue = .background) throws {
		guard let model = CoreDataWorkoutStore.model else {
	...
	public init(storeURL: URL, contextQueue: ContextQueue = .background, model: NSManagedObjectModel) throws {
		do {
			container = try NSPersistentContainer.load(name: CoreDataWorkoutStore.modelName, model: model, url: storeURL)
