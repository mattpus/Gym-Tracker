...
@Observable
@MainActor
final class EditCustomExerciseViewModel {
    let id: UUID
    var name: String
    ...
    private let editCustomExerciseUseCase: CustomExerciseEditing

    init(exercise: LibraryExerciseItem, editCustomExerciseUseCase: CustomExerciseEditing) {
        self.id = exercise.id
        self.name = exercise.name
        self.primaryMuscleGroup = exercise.primaryMuscleGroup
        self.secondaryMuscleGroups = Set(exercise.secondaryMuscleGroups)
        self.equipmentType = exercise.equipmentType
        self.editCustomExerciseUseCase = editCustomExerciseUseCase
    }
    ...
    func save(completion: @escaping (Bool) -> Void) {
        ...
        do {
            try editCustomExerciseUseCase.update(exercise)
            isSaving = false
            completion(true)
        } catch {
            self.error = error
